...
def add_file(): # files 변수에 사용할 이미지들을 저장
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\wjdtj\Desktop\Python Workspace")
...
def merge_image():
    ...
    # 이미지의 size -> size[0]: width, size[1]: height
    # widths = [x.size[0] for x in images]
    # heights = [x.size[1] for x in images]
    widths, heights = zip(*(x.size for x in images))
    # zip을 사용하면 두 리스트를 합치며, 그 형태는 [(0, 0), (1,1), ...]이다.
    # zip된 리스트를 분리할 때는 앞에 *을 붙이며, 선언된 리스트에 순서대로 삽입된다.
    ...
    y_offset = 0 # y 위치
    # for img in images:
    #     result_img.paste(img, (0, y_offset)) # 이미지를 (0, y_offset) 좌표에 붙여넣기
    ...
    dest_path = os.path.join(txt_dest_path.get(), "나도 포토.jpg") # 이미지 저장 경로 및 이름 설정
