...
character_width = chracter_size[0] # 캐릭터의 가로 크기
character_height = chracter_size[1] # 캐릭터의 세로 크기
character_x_pos = screen_width / 2 - (character_width/2) # 화면 가로의 절반에 위치(정중앙)
character_y_pos = screen_height - character_height # 화면 세로 가장 아래에
...
# 적 캐릭터
enemy = pygame.image.load("C:\\Users\\wjdtj\\Desktop\\Python Workspace\\pygame_basic\\enemy.png")
enemy_size = enemy.get_rect().size # 이미지의 크기를 구해옴
enemy_width = enemy_size[0] # 적의 가로 크기
enemy_height = enemy_size[1] # 적의 세로 크기
enemy_x_pos = screen_width / 2 - enemy_width/2 # 화면 가로의 절반에 위치(정중앙)
enemy_y_pos = screen_height/2 - enemy_height/2 # 화면 세로 가장 아래에
...
# 세로 경계값 처리
    ...
    #경과 시간을 1000으로 나누어 ms -> 초(s) 단위로 표시
