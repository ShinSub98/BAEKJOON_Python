...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        break
                ...
                # 리스트의 초기 인덱스 값은 0부터 시작하므로
                # ball_to_remove의 값을 -1로 초기화 한 후
                # 공과 무기가 충돌 시 ball_to_remove의 값을 해당 공의 인덱스 값으로 재초기화 해주면
                # ball_to_remove의 값이 -1보다 커지게 되고 해당 공의 인덱스 값과 같아지므로
                # 해당 공만 골라 삭제할 수 있다.

    # 충돌된 공/무기 삭제
    if ball_to_remove > -1:
        del balls[ball_to_remove]
        ball_to_remove = -1 # ball_to_remove의 값을 다시 -1로 초기화 함으로써 다시 재사용할 수 있도록 한다.
    ...
    timer = game_font.render("Play Time: "+str(int(total_time - elapsed_time))+"s", True, (0,0,0))
...
# 게임 오버 메시지
msg = game_font.render(game_result, True, (255, 255, 0)) #노란색 폰트 출력
